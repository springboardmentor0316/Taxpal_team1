          ...
          <form onSubmit={async (e) => {
            ...
            try {
              const res = await axios.post(
                ...
                { type: "income", description, amount: parseFloat(amount), category, date, notes },
                { headers: { ...getAuthHeaders() } }
              );
              const item = res.data?.item || { description, amount: parseFloat(amount), category, date, notes, type: "income" };
              onSave && onSave(item);
              onClose && onClose();
            } catch (err) {
              onSave && onSave({ description, amount: parseFloat(amount), category, date, notes, type: "income" });
              onClose && onClose();
            }
          }}>
            <h3>Add Income</h3>
